...
provider-read window. Ordinary process recovery preserves a clean durable
cursor, but every role-recovery epoch starts a fresh full generation. Exact
eligible provider/projection count equality then closes the interval in which
another default SMS app could have inserted or changed deep history beyond a
bounded head check.
