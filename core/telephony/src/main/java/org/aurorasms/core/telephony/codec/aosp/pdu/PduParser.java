 ...
 *
 * Modified by the AuroraSMS project in 2026. See third_party/aosp-mms/README.md.
 */

package org.aurorasms.core.telephony.codec.aosp.pdu;

import org.aurorasms.core.telephony.codec.aosp.ContentType;
import org.aurorasms.core.telephony.codec.aosp.InvalidHeaderValueException;
...
public class PduParser {
    ...
    private static final int MAX_PDU_BYTES = 1024 * 1024;
    private static final int MAX_PART_COUNT = 25;
    private static final int MAX_PART_HEADER_BYTES = 8192;
    private static final int MAX_WAP_STRING_BYTES = 2048;
    private static final int MAX_MULTIPART_DEPTH = 8;
    ...
    public PduParser(byte[] pduDataStream) {
        if (pduDataStream == null || pduDataStream.length == 0
                || pduDataStream.length > MAX_PDU_BYTES) {
            mPduDataStream = null;
        } else {
            mPduDataStream = new ByteArrayInputStream(
                    Arrays.copyOf(pduDataStream, pduDataStream.length));
        }
    }
        ...
        switch (messageType) {
            ...
            case PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF:
                ...
                if (ctTypeStr.equals(ContentType.MULTIPART_ALTERNATIVE)) {
                    if (mBody.getPartsNum() == 0) {
                        return null;
                    }
                    // "application/vnd.wap.multipart.alternative" takes its first part.
                    PduPart firstPart = mBody.getPart(0);
                    mBody.removeAll();
                    mBody.addPart(0, firstPart);
                    return retrieveConf;
                } else if (ctTypeStr.equals(ContentType.MULTIPART_MIXED)
    ...
    private PduBody parseParts(ByteArrayInputStream pduDataStream, int depth) {
        ...
        if (count < 0 || count > MAX_PART_COUNT) {
            return null;
        }
        ...
        for (int i = 0 ; i < count ; i++) {
            ...
            if (headerLength <= 0 || headerLength > MAX_PART_HEADER_BYTES
                    || dataLength < 0
                    || ((long) headerLength + (long) dataLength) > pduDataStream.available()) {
                return null;
            }
            ...
            /* Give otherwise valid anonymous parts a deterministic local content-location. */
            if ((null == part.getContentLocation())
                    && (null == part.getName())
                    && (null == part.getFilename())
                    && (null == part.getContentId())) {
                part.setContentLocation(
                        ("part-" + i).getBytes(StandardCharsets.US_ASCII));
            }
            ...
            if (dataLength > 0) {
                ...
                if (pduDataStream.read(partData, 0, dataLength) != dataLength) {
                    return null;
                }
    ...
    private static void log(String text) {
        // AuroraSMS never writes parser or message-derived details to logs.
    }
    ...
    protected static int parseUnsignedInt(ByteArrayInputStream pduDataStream) {
        ...
        while((temp & 0x80) != 0) {
            if (octets++ >= 5 || result > (Integer.MAX_VALUE >>> 7)) {
                return -1;
            }
            result = result << 7;
            result |= temp & 0x7F;
    ...
    protected static int parseValueLength(ByteArrayInputStream pduDataStream) {
         ...
         */
        if (pduDataStream == null) {
            throw new IllegalArgumentException("Missing PDU data");
        }
        int temp = pduDataStream.read();
        if (temp == -1) {
            throw new IllegalArgumentException("Truncated value length");
        }
        int first = temp & 0xFF;
    ...
    protected static EncodedStringValue parseEncodedStringValue(ByteArrayInputStream pduDataStream){
        ...
        int temp = pduDataStream.read();
        if (temp == -1) {
            throw new IllegalArgumentException("Truncated encoded string");
        }
        int first = temp & 0xFF;
        if (first == 0) {
    ...
    protected static byte[] parseWapString(ByteArrayInputStream pduDataStream,
        ...
        int temp = pduDataStream.read();
        if (temp == -1) {
            throw new IllegalArgumentException("Truncated WAP string");
        }
    ...
    protected static byte[] getWapString(ByteArrayInputStream pduDataStream,
        ...
        while((-1 != temp) && ('\0' != temp)) {
            if (out.size() >= MAX_WAP_STRING_BYTES) {
                return null;
            }
        ...
        if (temp == -1) {
            throw new IllegalArgumentException("Unterminated WAP string");
        }
    ...
    protected static int extractByteValue(ByteArrayInputStream pduDataStream) {
        ...
        int temp = pduDataStream.read();
        if (temp == -1) {
            throw new IllegalArgumentException("Truncated byte value");
        }
        return temp & 0xFF;
    }
    ...
    protected static int parseShortInteger(ByteArrayInputStream pduDataStream) {
        ...
        int temp = pduDataStream.read();
        if (temp == -1) {
            throw new IllegalArgumentException("Truncated short integer");
        }
        return temp & 0x7F;
    }
    ...
    protected static long parseLongInteger(ByteArrayInputStream pduDataStream) {
        ...
        int temp = pduDataStream.read();
        if (temp == -1) {
            throw new IllegalArgumentException("Truncated long integer");
        }
        int count = temp & 0xFF;
    ...
    protected static long parseIntegerValue(ByteArrayInputStream pduDataStream) {
        ...
        int temp = pduDataStream.read();
        if (temp == -1) {
            throw new IllegalArgumentException("Truncated integer value");
        }
        pduDataStream.reset();
    ...
    protected static int skipWapValue(ByteArrayInputStream pduDataStream, int length) {
        if (pduDataStream == null || length < 0 || length > pduDataStream.available()) {
            return -1;
        }
    ...
    protected static void parseContentTypeParams(ByteArrayInputStream pduDataStream,
         ...
         */
        if (pduDataStream == null || length == null || length <= 0
                || length > pduDataStream.available()) {
            throw new IllegalArgumentException("Invalid content-type parameters");
        }
        ...
        while(0 < lastLen) {
            ...
            if (param == -1) {
                throw new IllegalArgumentException("Truncated content-type parameters");
            }
            lastLen--;
    ...
    protected static byte[] parseContentType(ByteArrayInputStream pduDataStream,
        ...
        int temp = pduDataStream.read();
        if (temp == -1) {
            throw new IllegalArgumentException("Truncated content type");
        }
        pduDataStream.reset();
        ...
        if (cur < TEXT_MIN) {
            ...
            if (length < 0 || length > pduDataStream.available()) {
            ...
            int index = parseShortInteger(pduDataStream);
            if (index < 0 || index >= PduContentTypes.contentTypes.length) {
    ...
    protected static boolean parsePartHeaders(ByteArrayInputStream pduDataStream,
        ...
        if (pduDataStream == null || part == null || length <= 0
                || length > pduDataStream.available()) {
            return false;
        }
                ...
                switch (header) {
                    ...
                    case PduPart.P_CONTENT_DISPOSITION:
                        ...
                        if (contentDisposition) {
                            ...
                            if (len < 1 || len > pduDataStream.available()) {
                                return false;
                            }
                            pduDataStream.mark(1);
                            ...
                            if (thisStartPos - thisEndPos < len) {
                                ...
                                if (thisStartPos - thisEndPos < len) {
                                    ...
                                    if (skipWapValue(pduDataStream, last) < 0) {
                                        return false;
                                    }
                                }
                            }
