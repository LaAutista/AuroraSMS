...
TEST_METHOD="verifiedConversationWallpaperSurvivesHostForceStopAndColdTargetProcessRelaunch"
TEST_TARGET="$TEST_CLASS#$TEST_METHOD"
GATE_ARGUMENT="auroraEmulatorWallpaperColdRestart"
PHASE_ARGUMENT="auroraEmulatorWallpaperColdRestartPhase"
PENDING_FILE_NAME=".pending-00000000-0000-0000-0000-000000000002"
PENDING_RELATIVE_PATH="no_backup/appearance/wallpapers/$PENDING_FILE_NAME"
...
if ! printf '%s\n' "${DEVICES[@]}" \
    ...
    printf 'Requested cold-restart emulator is not authorized: %s\n' "$DEVICE_SERIAL" >&2
    exit 1
fi
...
if [[ "$DEVICE_SERIAL" != emulator-* || "$IS_EMULATOR" != "1" ]]; then
    printf 'The cold-restart wallpaper smoke refuses physical device: %s\n' \
        "$DEVICE_SERIAL" >&2
    exit 1
fi
if [[ "$HARDWARE" != "ranchu" && "$HARDWARE" != "goldfish" ]]; then
    printf 'The cold-restart wallpaper smoke requires a ranchu or goldfish emulator.\n' >&2
    exit 1
fi

SDK_LEVEL="$("${ADB[@]}" shell getprop ro.build.version.sdk | tr -d '\r')"
if [[ "$SDK_LEVEL" != "36" ]]; then
    printf 'The cold-restart wallpaper smoke requires API 36 exactly.\n' >&2
    exit 1
fi
...
force_stop_and_require_absent() {
    ...
    printf 'AuroraSMS process remained alive after host force-stop.\n' >&2
    return 1
}
...
start_prepared_target_and_arm_pending() {
    local attempt pid
    ...
    if [[ -z "$pid" ]]; then
        printf 'AuroraSMS did not expose a live prepared process for host force-stop.\n' >&2
        return 1
    fi
    for attempt in {1..80}; do
        if "${ADB[@]}" shell run-as "$APP_PACKAGE" test ! -e "$PENDING_RELATIVE_PATH"; then
            break
        fi
        sleep 0.1
    done
    if ! "${ADB[@]}" shell run-as "$APP_PACKAGE" test ! -e "$PENDING_RELATIVE_PATH"; then
        printf 'The prepared process did not reconcile the initial pending fixture.\n' >&2
        return 1
    fi
    "${ADB[@]}" shell run-as "$APP_PACKAGE" touch "$PENDING_RELATIVE_PATH"
    if ! "${ADB[@]}" shell run-as "$APP_PACKAGE" test -f "$PENDING_RELATIVE_PATH"; then
        printf 'The host could not re-arm the exact pending wallpaper fixture.\n' >&2
        return 1
    fi
    if [[ "$(current_target_pid)" != "$pid" ]]; then
        printf 'AuroraSMS process changed while the host armed the pending fixture.\n' >&2
        return 1
    fi
    ARMED_TARGET_PID="$pid"
...
force_stop_live_target() {
    ...
    if [[ -z "$expected_pid" || "$current_pid" != "$expected_pid" ]]; then
        printf 'The exact prepared target process was not alive before host force-stop.\n' >&2
        return 1
    fi
    force_stop_and_require_absent
...
run_phase() (
    ...
    phase="$1"
    output="$2"
    printf 'Running cold-restart wallpaper phase: %s\n' "$phase"
    set +e
    ...
    if [[ "$instrumentation_status" -ne 0 ]] || \
        ...
        printf 'Cold-restart wallpaper phase did not pass exactly one test: %s\n' "$phase" >&2
        return 1
    fi
)
...
if ! target_matches_build; then
    printf 'The installed target APK does not match the cold-restart smoke build.\n' >&2
    printf 'Update it separately, verify preserved state, and then rerun this script.\n' >&2
    exit 1
fi
...
cleanup() {
    ...
    if [[ "$cleanup_ok" -ne 1 ]]; then
        status=1
        printf 'Exact recovery cleanup failed; preserving the test APK and logs.\n' >&2
    ...
    if ! target_matches_build; then
        printf 'The target APK was removed or changed by cold-restart cleanup.\n' >&2
        status=1
    fi
    ...
    if ! permissions_after="$(permission_state)"; then
        ...
        printf 'The cold-restart smoke changed AuroraSMS permission state.\n' >&2
        status=1
    fi
    if [[ "$role_after" != "$BEFORE_ROLE" ]]; then
        printf 'The cold-restart smoke changed the SMS role holder.\n' >&2
        status=1
    fi
    if [[ "$status" -eq 0 ]]; then
        rm -rf "$WORK"
        printf 'Cold-restart cleanup preserved the target APK, SMS role, and permissions.\n'
    else
        printf 'Cold-restart evidence logs retained at: %s\n' "$WORK" >&2
    fi
    exit "$status"
}
trap cleanup EXIT
...
# Recover only a checkpoint owned by an interrupted earlier invocation, then begin from a
# processless target. App startup may reconcile preexisting managed state before baseline capture.
...
start_prepared_target_and_arm_pending
force_stop_live_target "$ARMED_TARGET_PID"
...
printf 'Cold-restart wallpaper prepare, verification, and cleanup each passed exactly one test.\n'
